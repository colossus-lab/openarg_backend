...
| [`001d-analysis/`](./001d-analysis/spec.md) | `analyst`, `policy` (optional) | Streaming LLM synthesis, chunk buffering, deterministic + LLM-generated chart building, GeoJSON map extraction, META-tag parsing for citations. (Internal `confidence` is still computed but no longer surfaced in the API response — commit `acc884a` removed it from `SmartQueryV2Response`, the WS `complete` event and the public `/ask`.) |
| [`001e-finalization/`](./001e-finalization/spec.md) | `finalize` | Source/document extraction, audit log, fire-and-forget cache write + memory update via `_background_tasks` registry, `duration_ms` computation, terminal streaming event with hardcoded key whitelist. Every terminal node (`finalize`, `cache_reply`, `fast_reply`, `clarify_reply`) writes exactly one `query_analytics` row inline via `record_terminal_analytics` — telemetry coverage is now ~100% across the pipeline (BUG-016/017, `a2b2288`). |
