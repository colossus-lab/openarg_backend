...
- **FR-038d**: If `_safe_send_json()` has to fall back from `json_default` to `to_json_safe`, it MUST emit a warning naming the top-level payload fields that forced the fallback so the producing node/connector can be traced from logs.
...
- **[DEBT-018]** — ~~**Origin of non-primitive values in finalized state is not instrumented**~~ **PARTIALLY FIXED 2026-04-12**: `_safe_send_json()` now logs which top-level payload fields required the `to_json_safe` fallback. Nested offending leaves inside a large object are still not pinpointed automatically.
