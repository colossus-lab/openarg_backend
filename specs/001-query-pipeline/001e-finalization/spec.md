...
- **FR-036a**: Finalize MUST return ``chart_data``, ``map_data``, ``citations``, ``sources`` and ``documents`` in its return dict so the LangGraph `updates` stream can forward them to the browser's `complete` event. LangGraph passes only the node's return dict to the update handler — anything the finalize node reads from state but does not re-emit is silently lost. Regression-tested by a unit test on `finalize_node` output keys (FIX-010, 2026-04-11). (`confidence` was previously in this list but commit `acc884a` removed it from the API surface; the field still flows internally for logging but never reaches the `complete` event — see FR-021a in `001d-analysis/spec.md`.)
- **FR-036a1**: The WebSocket router MUST emit the browser `complete` event only from terminal nodes (`finalize`, `cache_reply`, `fast_reply`, `clarify_reply`). Intermediate updates that happen to contain `clean_answer` (for example `analyst`) are not terminal and MUST NOT close the stream early, otherwise the browser receives `answer` without the finalized `sources/documents/chart_data` payload. `clarify_reply` was added to this set 2026-05-14.
- **FR-036t (BUG-016/017, commit `a2b2288`)**: every terminal node (`finalize`, `cache_reply`, `fast_reply`, `clarify_reply`) MUST write exactly one row to `query_analytics` via `record_terminal_analytics`, awaited **inline** (not fire-and-forget — an earlier background-task implementation raced request teardown and silently lost ~2/3 of writes). `finalize` is the single logger for every successful data flow; the other terminals log their own short-circuit paths. NL2SQL no longer writes from `save_success_node` (that double-counted sandbox queries and missed connector/mart flows); the served table travels via `DataResult.metadata.served_table`.
- **FR-036t1 (P1 retry, commit `f3f18d0`)**: `save_query_attempt` MUST retry the INSERT once after a 0.5 s backoff if the first attempt raises. DDL setup (`CREATE TABLE IF NOT EXISTS`) lives outside the retry loop. Round v4.2 showed ~11% of rows missed `query_analytics` (R002/R014/C5 in a 27-call batch) due to transient pool/lock failures; the retry pushes coverage toward 100%.
...
- **FR-036f1**: Citation labels that include a calendar date only as source context (for example `14/04/2026` in a DolarApi citation title) MUST NOT be treated as unsupported numeric claims. Those date tokens are source metadata, not evidence-bearing quantitative assertions, and MUST NOT by themselves force the confidence chip down.
...
- **[DEBT-010]** — ~~**WebSocket stream_mode handling has hardcoded node names**~~ **PARTIALLY FIXED 2026-04-12**: the router now treats `updates` payloads with `clean_answer` as terminal complete events instead of relying only on specific node names. The stream loop still assumes LangGraph `updates` mode semantics and could be tightened further with a more explicit terminal-node contract.
...
- **[DEBT-014]** — ~~**`_get_or_compile_graph()` uses `threading.Lock` in async context**~~ **FIXED 2026-04-13**: the router now guards the compiled-graph cache with an `asyncio.Lock`, and `_get_or_compile_graph()` is awaited by request handlers and tests instead of mixing a blocking thread lock into the event loop path.
...
- **[DEBT-019]** — ~~**Lazy checkpointer initialisation in request handlers can drift from the app lifecycle**~~ **FIXED 2026-04-13**: request handlers now reuse the app-scoped `_checkpointer` that is warmed in startup and closed on shutdown; they no longer call `_get_checkpointer()` on the request path, so persistence ownership stays aligned with `app_factory.lifespan`.
...
- **[DEBT-018]** — **Origin of non-primitive values in finalized state is still not instrumented at the producer boundary.** The WebSocket send path is now cheaper because it normalises in one pass, but the system still does not pinpoint which upstream connector/node first introduced a non-primitive leaf into the final state.
