...
- **[RESOLVED CL-002]** — **Verified directly in the production DB (openarg_staging) on 2026-04-10**. Cache hit rate effectively **0%**. Data: `query_cache` has **1 single entry** (created on 2026-04-06, already expired). Total historical traffic: ~28 user queries across all channels (23 messages + 3 api_usage + 2 user_queries). The cache is not serving hits because **the system has almost no traffic** — it is not a code bug. **Suspicious sub-finding**: ~28 queries but only 1 cache write — suggests that many queries don't reach the `finalize` node (fast_reply, clarify, NL2SQL inline path don't write cache) or that `write_cache` fire-and-forget fails silently without retry (`001-query-pipeline/DEBT-003`). **Revisit when** there is real traffic volume (>100 queries/day) — today there is no signal to optimize.
- **[RESOLVED CL-003]** — The allowlist lives in the frontend (`openarg_frontend/src/lib/authOptions.ts` NextAuth signIn callback), per-environment policy: **staging** (`REDACTED_STAGING`) maintains a private alpha with ~28 active emails + `OPEN_BETA=false`; **production** (`openarg.org`) has a bypass (`OPEN_BETA=true` or empty allowlist) for public access. Verified 2026-04-10 in the staging server env. General opening plan: staging stays private, prod is public under operator policy.
