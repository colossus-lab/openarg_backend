...
- **[RESOLVED CL-001]** — `expires_at` is effectively **dead code**. It exists in the `ApiKey` entity, the `api_keys` table column, and is checked by `api_key_service.verify_api_key` (line 69), but no endpoint or code path ever sets it: the `POST /developers/keys` handler constructs `ApiKey(..., plan="free")` without an `expires_at`, and there is no rotation/expiration task. All active keys live until explicitly revoked. See `src/app/presentation/http/controllers/developers/developers_router.py:65-72` and `src/app/application/api_key_service.py:69`. (resolved 2026-04-11 via code inspection)
