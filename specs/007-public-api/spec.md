...
- **[NEEDS CLARIFICATION CL-001]** — Is the `expires_at` field on API keys used? `verify_api_key` checks it but no endpoint sets it.
...
- **[NEEDS CLARIFICATION CL-004]** — Rate limit headers (`X-RateLimit-*`): are they returned consistently? Not visible.
