...
- **DEBT-015-001**: `openarg.populate_catalog_embeddings` is an explicit batch task, not an incremental worker. New `catalog_resources` rows stay `embedding=NULL` (and `domain`/`subdomain`/`taxonomy_key` NULL) until operators run it or future incremental hooks land. **Coverage update 2026-05-09**: 12,995 / 25,755 rows (50.5 %) carry an embedding — over the threshold that DEBT-016-001 needed before wiring HNSW into mart discovery. The remaining ~50 % are mostly recently-landed resources awaiting the next batch run.
