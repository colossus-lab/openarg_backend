...
- **FR-028e1** *(Sprint 38, 2026-05-07)*: A configurable portal allow-list (`OPENARG_HEAVY_PORTALS`, default `cordoba_estadistica`) MUST force-route every dataset of those portals to the heavy lane unconditionally — no preview/HEAD probe required. Implemented as `_HEAVY_PORTALS` frozenset + `_reroute_to_heavy(f"force_heavy_portal:{portal}")` short-circuit at the top of `collect_dataset`. Reason: portals dominated by GIS/shapefile bundles consistently OOM the normal lane and burned-portal lockouts kept blocking their drain.
...
- **FR-035** *(Sprint 38, 2026-05-07)*: ZIP archives containing a mix of oversized and parseable entries MUST adopt a *skip-and-continue* policy: oversized entries are logged and skipped, the run proceeds with the valid ones. Only when ALL entries are oversized may the collector abort with `zip_entry_too_large`. Reason: GTFS `stop_times.txt` and `Bicicletas Públicas` `trips_2022.csv` (>500 MB single members) caused entire ZIPs to be rejected even though sibling members held the data we cared about. Implemented in the `MultiFileExpander` consumer in `collect_dataset` — see logic guarded by `expansion.skipped_oversized and not expanded_member_names` (abort) vs `expansion.skipped_oversized` alone (warn + continue).
- **FR-036** *(Sprint 38, 2026-05-07)*: A configurable bypass list (`OPENARG_BYPASS_BURNED_PORTALS`, default empty) MUST let named portals escape the `bulk_collect_all` `burned_portals` exclusion (>50% `permanently_failed` in the rolling 24h window). Without this knob, the only way to drain a portal that was previously burned by a since-fixed runtime issue (OOM, short timeout, etc.) was to wait for the 24h window to slide. Implemented as a SQL placeholder (`{bypass_clause}`) injected as `AND d2.portal <> ALL(:bypass_portals)` inside the `burned_portals` CTE. Operators set the env var, no schema or beat change required.
- **FR-037** *(Sprint 38, 2026-05-07)*: `reset_failed_collectors` MUST flip `datasets.is_cached=false` for every reset row, not only update `cached_datasets`. Without that flip, the natural `bulk_collect_all` filter `WHERE d.is_cached = false` keeps the reset rows out and the operation is a silent no-op for any dataset that had ever reached `ready` and later degraded. The task now returns `{reset, uncached}` so observability covers both updates.
...
- **SC-019** *(Sprint 38, 2026-05-07)*: Datasets like GTFS bundles whose only blocker was a single oversized member (e.g. `stop_times.txt`) MUST land as `ready` after the skip-and-continue change. 14/22 staging `zip_entry_too_large` cases recovered in the first sweep after deploy.
- **SC-020** *(Sprint 38, 2026-05-07)*: A reset of a previously-burned portal (via `OPENARG_BYPASS_BURNED_PORTALS` + `reset_failed_collectors` + force-heavy reroute) MUST drain the backlog without manual `celery call` storms. Validated 2026-05-07 on `cordoba_estadistica`: 8794 datasets, 90.9% reached `ready` end-to-end through the chained natural drain.
...
- **[DEBT-011]** — **Collector worker stability under large wide bundles still depends on deployment tuning**. The ZIP/CSV path now streams in chunks, but operator-facing defaults such as worker concurrency and per-child memory limits still need environment-specific tuning to avoid SIGKILL churn on staging/prod. *Update 2026-05-07*: staging deploy spec hardened — `worker-collector` now runs with `-c 8 --soft-time-limit=1200 --time-limit=1380` and the previous `--max-memory-per-child=400000` cap was removed. The earlier combination (`-c 16` × 400 MB child cap × 600 s soft-limit) was producing kernel SIGKILL OOMs at scale and left **1,776 datasets** stuck in `orchestration_recovery_loop` ("Exhausted retries while stuck in downloading"). A reset+rerun after the config change drained them. Production still pending the same tuning.
- **[DEBT-012]** *(Sprint 38, 2026-05-07)* — **`error_category` mis-classification for OOM-loop perm-failed rows**. The same error message "Exhausted retries while stuck in downloading" is recorded under three different categories (`orchestration_recovery_loop`, `validation_failed`, `materialize_disk_full`) depending on which code path drove the row to terminal. Burned-portal aggregates and operator dashboards lose signal because of it. Fix should be a single `_classify_error_category` rule that maps that exact message to `orchestration_recovery_loop` regardless of which transition wrote `status='permanently_failed'`. Cf. spec 014 §3 closed taxonomy.
