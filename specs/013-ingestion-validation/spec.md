...
- **DEBT-013-003 (CLOSED 2026-05-09)**: `force_recollect_separator_mismatches` is now part of the weekly beat schedule (`force-recollect-separator-mismatches-weekly`, Sundays 05:00 ART, after the cleanup-orphan / cleanup-empty-raw tasks). No more manual dispatch required. The task targets `cached_datasets.status='ready'` rows that still carry an open critical finding from the `separator_mismatch` detector; on each run those rows are flipped back to `pending` and re-collected. If the separator sniffer reads the data cleanly the next time around, the finding is closed automatically by `close_resolved_findings_task` (runs every 15 min); if it still parses poorly, the post-parse gate aborts the row as `parser_invalid` and the state machine moves it to its terminal lane.
