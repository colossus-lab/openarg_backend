# Plan: SQL Sandbox — Read-only Executor (As-Built)
...
| Infrastructure Adapter | `PgSandboxAdapter` | `infrastructure/adapters/sandbox/pg_sandbox_adapter.py` |
| Infrastructure Helper | `table_validation.py` | `infrastructure/adapters/sandbox/table_validation.py` |
| Presentation | `sandbox_router.py` (raw-SQL endpoints) | `presentation/http/controllers/sandbox/sandbox_router.py` |
...
(`POST /api/v1/sandbox/ask` is owned by `010b-nl2sql`.)
...
- `presentation/http/controllers/sandbox/sandbox_router.py` (raw-SQL handlers)
