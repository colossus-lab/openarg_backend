...
- **[DEBT-008]** — **Unversioned `cache_*` table schemas** (no Alembic migrations). Created with `df.to_sql(..., if_exists="replace")`. Impact: silent schema drift; the SQL sandbox can break on upstream changes. **Partially mitigated 2026-04-10**: `collector_tasks._to_sql_safe()` now calls `_detect_schema_drift()` before every `if_exists="replace"` write; when existing columns are dropped, renamed, or retyped, a structured `schema_drift_detected` warning is logged and a `schema_drift:<table>` metric is incremented via `MetricsCollector`. The write still proceeds (no blocking behavior) but operators now get visibility instead of silent "column does not exist" errors hours later. Deep fix (versioned schemas + migrations) still pending.
