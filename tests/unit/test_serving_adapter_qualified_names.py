...
def _ok(sql: str, schema: str, table: str) -> bool:
    ...
    ok, _ = is_pure_select_for_relation(sql, expected_schema=schema, expected_table=table)
    return ok
...
def test_sql_reference_match_requires_schema_for_raw() -> None:
    assert _ok('SELECT * FROM raw."cache_demo__v1"', "raw", "cache_demo__v1")
    # Unqualified ref to a raw-only table must NOT count — medallion
    # schemas require explicit qualification.
    assert not _ok('SELECT * FROM "cache_demo__v1"', "raw", "cache_demo__v1")
...
def test_h1_union_to_api_keys_is_rejected() -> None:
    ...
    sql = "SELECT id FROM mart.flujo_vehicular_peajes_caba UNION ALL SELECT id::text FROM api_keys"
...
def test_h1_join_to_successful_queries_is_rejected() -> None:
    ...
    sql = "SELECT m.id, q.question FROM mart.foo m JOIN successful_queries q ON true"
    ok, reason = is_pure_select_for_relation(sql, expected_schema="mart", expected_table="foo")
    assert not ok
    assert reason is not None
...
def test_h1_cte_hides_internal_table() -> None:
    ...
    sql = "WITH x AS (SELECT question FROM query_analytics) SELECT * FROM mart.foo, x"
    ok, reason = is_pure_select_for_relation(sql, expected_schema="mart", expected_table="foo")
    assert not ok
    assert reason is not None
...
def test_h2_cross_schema_reference_is_rejected() -> None:
    ...
    sql = "SELECT * FROM mart.foo UNION ALL SELECT * FROM mart.other_mart"
    ok, reason = is_pure_select_for_relation(sql, expected_schema="mart", expected_table="foo")
    assert not ok
    assert reason is not None
...
def test_h2_cte_alias_does_not_false_positive() -> None:
    ...
    sql = "WITH summary AS (SELECT id FROM mart.foo WHERE id > 0) SELECT * FROM summary"
    ok, reason = is_pure_select_for_relation(sql, expected_schema="mart", expected_table="foo")
    assert ok, reason
...
async def test_query_uses_resource_schema_layer() -> None:
    ...
    adapter.get_schema = AsyncMock(  # type: ignore[method-assign]
        return_value=Schema(
            columns=["valor"], column_types={"valor": "integer"}, layer=ServingLayer.MART
        )
    )

    rows = await adapter.query("mart::series_economicas", "SELECT * FROM mart.series_economicas")
...
async def test_query_rejects_relation_mismatch() -> None:
    ...
    adapter.get_schema = AsyncMock(  # type: ignore[method-assign]
        return_value=Schema(
            columns=["valor"], column_types={"valor": "integer"}, layer=ServingLayer.RAW
        )
    )
...
async def test_debt_016_003_bug2_explain_mart_short_circuits() -> None:
    ...
    assert not any("catalog_resources" in s for s in captured_sqls), (
        "explain(mart::*) leaked into catalog_resources — DEBT-016-003 regression"
    )
