...
def _upload_to_s3(
    content: bytes, portal: str, dataset_id: str, filename: str,
) -> str:
...
@celery_app.task(
    name="openarg.retry_s3_uploads", bind=True,
)
...
def upload_dataset_to_s3(
    ...
    try:
        ...
        if not row:
            logger.warning(
                "Dataset %s not found or not ready", dataset_id,
            )
        ...
        with httpx.Client(timeout=120.0) as client:
            resp = client.get(
                row.download_url, follow_redirects=True,
            )
        ...
        s3_key = _upload_to_s3(
            resp.content, row.portal, dataset_id, filename,
        )
        ...
        logger.error(
            "S3 upload timed out for dataset %s", dataset_id,
        )
        raise
    except Exception as exc:
        logger.exception(
            "S3 upload failed for dataset %s", dataset_id,
        )
